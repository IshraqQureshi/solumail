    ...
    public function Paypal_Capture_Order_Payment($order_id)
    {
        if (@$order_id && @$this->access_token) {
            ...
            if (@$order->id) {
                ...
                $curl = curl_init();
                curl_setopt($curl, CURLOPT_URL, "https://api-m.sandbox.paypal.com/v2/checkout/orders/$order_id/capture");
                curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
                ...
                if (@$result->id) {
                    if (@$result->purchase_units[0]->payments->captures[0]->id) {
                        ...
                        $curl = curl_init();
                        curl_setopt($curl, CURLOPT_URL, "https://api-m.sandbox.paypal.com/v2/payments/captures/$capture_id");
                        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
                        curl_setopt($curl, CURLOPT_ENCODING, '');
                        curl_setopt($curl, CURLOPT_MAXREDIRS, 10);
                        curl_setopt($curl, CURLOPT_TIMEOUT, 0);
